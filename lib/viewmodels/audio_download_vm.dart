...
class AudioDownloadVM extends ChangeNotifier {
  ...
  void deleteAudioPhysicallyAndFromPlayableAudioListOnly({
    ...
    enclosingPlaylist.removePlayableAudio(playableAudio: audio);
  ...
  ///
  /// The playlist json file is of course updated.
  void deleteAudioPhysicallyAndFromAllAudioLists({required Audio audio}) {
    DirUtil.deleteFileIfExist(pathFileName: audio.filePathName);
  ...
  void updatePlaylistJsonFiles({
    ...
    if (restoringPlaylistsCommentsAndSettingsJsonFilesFromZip) {
      ...
      // restored playlist
      initialListOfPlaylistCopy = _listOfPlaylist
          .map((Playlist playlist) => playlist.copy())
          .toList();
    }
    ...
    List<dynamic> orderedPlaylistTitleLst =
        _settingsDataService.get(
          settingType: SettingType.playlists,
    ...
    for (Playlist playlistCopy in listOfPlaylistCopy) {
      ...
      Playlist correspondingOriginalPlaylist = _listOfPlaylist.firstWhere(
        (element) => element == playlistCopy,
      );

      String playlistCopyDownloadHomePath = path.dirname(
        playlistCopy.downloadPath,
      );
      ...
      if (updatePlaylistPlayableAudioList) {
        removedPlayableAudioNumber = correspondingOriginalPlaylist
            .updatePlayableAudioLst();
      }
