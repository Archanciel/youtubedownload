...
enum SettingType { appTheme, language, playlists, dataLocation, formatOfDate }
...
enum DataLocation { appSettingsPath, playlistRootPath }
...
class SettingsDataService {
  ...
  final Map<SettingType, Map<dynamic, dynamic>> _settings = {
    ...
    SettingType.formatOfDate: {FormatOfDate.formatOfDate: 'dd/MM/yyyy'},
  };
  ...
  void saveSettingsToFile({required String jsonPathFileName}) {
    ...
    try {
      if (settingsJsonFileExist) {
        ...
        decodedSettings.forEach((key, value) {
          ...
          final subSettings = (value as Map<String, dynamic>).map((
            subKey,
            subValue,
          ) {
            return MapEntry(
  ...
  void savePlaylistTitleOrder({required String directory}) {
  ...
  void _saveSettings() {
    String applicationPath = DirUtil.getApplicationPath(isTest: _isTest);
    saveSettingsToFile(
      jsonPathFileName:
  ...
  dynamic _parseJsonValue(List<dynamic> enumValues, String stringValue) {
    if (stringValue.startsWith('[') && stringValue.endsWith(']')) {
      List<String> stringList = stringValue
